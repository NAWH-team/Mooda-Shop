  ...
  const login = () => {
    try {
      if (password && email) {
        ...
        axios.post('http://localhost:8080/brand/signin', obj)
          .then((res) => {
            
            res.data.existingbrand.type = "brand"
            window.localStorage.setItem('current',JSON.stringify(res.data.existingbrand));
